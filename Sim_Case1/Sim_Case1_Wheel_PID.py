...
def main():
    ...
    pidA = PIDController(-255, 255)
    pidB = PIDController(-255, 255)
    ...
    try:
        while True:
            ...
            error = np.array([vel_mag[0] - v, vel_mag[1] - w])

            preoutput = Helpers.CenterVel_to_WheelVel(error, 15.2, 8.1)

            pid1 = pidA.update(left_encoder.get_omega_rad(), 1, 0, 0, preoutput[0], dur)
            pid2 = pidB.update(right_encoder.get_omega_rad(), 1, 0, 0, preoutput[1], dur)

            output = Helpers.Wheel_Omega_to_PWM(pid1, pid2, 1)
