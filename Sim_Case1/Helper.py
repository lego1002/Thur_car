...
class Helpers:
    ...
    @staticmethod
    def to_deg_and_round(a: np.array, b):
        if len(a) != 3: return
    ...
    @staticmethod
    def CenterVel_to_WheelVel(a: np.array, wheel_base, wheel_radius):
        if len(a) != 2: return

        v_left = a[0] - (wheel_base / 2.0) * a[1]
        v_right = a[0] + (wheel_base / 2.0) * a[1]

        omega_left = v_left / wheel_radius
        omega_right = v_right / wheel_radius

        return np.array([omega_left, omega_right])
