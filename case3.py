...
def simulate_case3(times, v_target, omega_target):
    T_end = times[-1] + 1
    time = np.arange(0, T_end + T, T)
    ...
    for i, t in enumerate(time):
        if t <= times[0]:
            ...
            v[i] = v_target * (1 - (t - times[5]) / (times[6] - times[5]))
        else:
            v[i] = 0
        ...
        if t <= times[1]:
            ...
            omega[i] = omega_target * (t - times[1]) / (times[2] - times[1])
        elif t <= times[3]:
            omega[i] = omega_target
        elif t <= times[4]:
            omega[i] = omega_target * (1 - (t - times[3]) / (times[4] - times[3]))
        else:
            omega[i] = 0
    ...
    for i in range(1, len(time)):
        ...
        dx_enc = dx + np.random.normal(0, 0.001)
        dy_enc = dy + np.random.normal(0, 0.001)
        x_enc.append(x_enc[-1] + dx_enc)
        y_enc.append(y_enc[-1] + dy_enc)
...
def update_plot():
    ...
    if sim_index < len(time):
        ...
        ax4.plot(time[:sim_index], omega[:sim_index], label='omega(t)' if sim_index == 1 else "")
...
def run_simulation():
    ...
    time, x, y, x_enc, y_enc, v, omega, v_l, v_r, v_l_enc, v_r_enc, v_enc, omega_enc = simulate_case3(times, v_target, omega_target)
    sim_index = 0
    is_running = True
...
root = tk.Tk()
root.title("Case 3 Odometry 模擬 UI（排版優化＋圖標題）")
...
entries_t = []
tk.Label(control_frame, text="時間點 t1~t7（v/ω 段落控制）").grid(row=0, column=0, columnspan=8)
for i in range(7):
    tk.Label(control_frame, text=f"t{i+1}").grid(row=1, column=i)
    et = tk.Entry(control_frame, width=6)
    et.insert(0, str(i + 1))
    et.grid(row=2, column=i)
    entries_t.append(et)
...
btn_start.grid(row=4, column=0, padx=5)
btn_pause = tk.Button(control_frame, text="暫停模擬", command=pause_simulation)
btn_pause.grid(row=4, column=1, padx=5)
